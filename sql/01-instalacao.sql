...
select
  -- ⚠️ FUSO DA CAMPANHA: Rondônia é UTC−4 (America/Porto_Velho).
  -- ⚠️ A VÍRGULA MORAVA DENTRO DO COMENTÁRIO ("-- FUSO DA CAMPANHA,"),
  --    e sem ela `as dia count(*)` é erro de sintaxe: a instalação
  --    inteira caía no SQL Editor. Veio assim do modelo.
...
select
  -- ⚠️ FUSO DA CAMPANHA: Rondônia é UTC−4 (America/Porto_Velho).
  -- ⚠️ A vírgula morava dentro do comentário — ver a mesma nota em
  --    20260819120200_metricas.sql.
  (criado_em at time zone 'America/Porto_Velho')::date as dia,
  count(*) filter (where tipo = 'pagina_vista')          as viram_pagina,
  count(*) filter (where tipo = 'rolou_50')              as rolaram_metade,
